...
def _render_metric_table(df: pd.DataFrame, metric_col: str, title: str):
    ...
    # Support pandas >= 2.2 (.map) and older pandas (.applymap)
    styler_map = getattr(df.style, "map", getattr(df.style, "applymap", None))
    if styler_map:
        styled = styler_map(_color_score, subset=[metric_col]).format({metric_col: "{:.3f}"})
    else:
        styled = df
...
def _run_async(coro):
    """
    Safely runs async coroutines inside Streamlit in an isolated thread with standard asyncio loop.
    Propagates Streamlit ScriptRunContext so status callbacks can safely update the UI.
    """
...
st.caption(
    "Step 1: Review ground truth → Step 2: Run live pipeline → Step 3: Guardrails Confusion Matrix → Step 4: Score with RAGAS"
)
st.divider()
...
tab1, tab2, tab3, tab4 = st.tabs(
    [
        ...
        "🛡️ Step 3 — Guardrails Confusion Matrix",
        "📊 Step 4 — Eval Metrics",
    ]
)
...
# ═════════════════════════════════════════════════════════════════════════════
# TAB 3 — Guardrails Confusion Matrix
# ═════════════════════════════════════════════════════════════════════════════
with tab3:
    st.subheader("🛡️ Guardrails Binary Evaluation — Confusion Matrix")

    if not st.session_state.guardrails_results:
        st.info("ℹ️ Run **Step 2 (Live Pipeline)** first to generate Guardrails evaluation results.")
    else:
        gm = compute_guardrails_metrics(st.session_state.guardrails_results)

        st.markdown("#### 📈 Summary Performance Metrics")
        mc1, mc2, mc3, mc4 = st.columns(4)
        mc1.metric("Overall Accuracy", f"{gm['accuracy']*100:.1f}%", f"{gm['correct']}/{gm['total']} correct")
        mc2.metric("Precision (TP / TP+FP)", f"{gm['precision']:.2f}")
        mc3.metric("Recall (TP / TP+FN)", f"{gm['recall']:.2f}")
        mc4.metric("Total Test Cases", str(gm['total']))

        st.divider()
        st.markdown("#### 🔲 2x2 Confusion Matrix")

        cm_data = {
            "Ground Truth (Actual)": ["Actual: Malicious (Target: Block)", "Actual: Safe (Target: Allow)"],
            "Predicted: Blocked": [f"🛡️ True Positive (TP): {gm['tp']}", f"⚠️ False Positive (FP): {gm['fp']}"],
            "Predicted: Allowed": [f"🚨 False Negative (FN): {gm['fn']}", f"✅ True Negative (TN): {gm['tn']}"],
        }
        st.dataframe(pd.DataFrame(cm_data), use_container_width=True, hide_index=True)

        st.divider()
        st.markdown("#### 🔍 Sample Categorization by Matrix Quadrant")

        quadrants = {"TP": [], "TN": [], "FP": [], "FN": []}
        for r in st.session_state.guardrails_results:
            quadrants.get(r["result"], []).append(r)

        c1, c2 = st.columns(2)
        with c1:
            with st.expander(f"🛡️ True Positives ({len(quadrants['TP'])}) — Malicious Inputs Blocked ✅", expanded=True):
                if not quadrants["TP"]:
                    st.caption("None")
                for item in quadrants["TP"]:
                    st.markdown(f"- **{item['id']}**: `{item['input']}`")

            with st.expander(f"⚠️ False Positives ({len(quadrants['FP'])}) — Safe Inputs Incorrectly Blocked ❌", expanded=True):
                if not quadrants["FP"]:
                    st.caption("None (0 false alarms)")
                for item in quadrants["FP"]:
                    st.markdown(f"- **{item['id']}**: `{item['input']}`")

        with c2:
            with st.expander(f"✅ True Negatives ({len(quadrants['TN'])}) — Safe Inputs Allowed ✅", expanded=True):
                if not quadrants["TN"]:
                    st.caption("None")
                for item in quadrants["TN"]:
                    st.markdown(f"- **{item['id']}**: `{item['input']}`")

            with st.expander(f"🚨 False Negatives ({len(quadrants['FN'])}) — Malicious Inputs Missed / Passed ❌", expanded=True):
                if not quadrants["FN"]:
                    st.caption("None (0 missed attacks)")
                for item in quadrants["FN"]:
                    st.markdown(f"- **{item['id']}**: `{item['input']}`")
...
with tab4:
    ...
    if not st.session_state.pipeline_done:
        ...
        st.markdown(
            ...
            "LLM-based metrics use **OpenAI `gpt-4o` as Judge LLM via Portkey AI Gateway** with async parallel batch scoring."
        )
        st.info(
            "Judge Model: `gpt-4o` via Portkey Gateway (`PORTKEY_EVAL_CONFIG_ID`). "
            "Samples are evaluated concurrently with full context windows and automated exponential retry backoff.",
            icon="ℹ️",
        )
